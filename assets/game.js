...
function startGame() {
  ...
  updateHud();
  if(document.documentElement.requestFullscreen){document.documentElement.requestFullscreen().catch(()=>{});} requestAnimationFrame(loop);
}
function updateHud() {
...
let touchStart = null;
canvas.addEventListener('click',()=>{if(G) togglePause();});
...
function drawBackdrop() {
  ...
  if (save.selectedBackground === "custom" && customBgImg && customBgImg.complete && customBgImg.naturalWidth) {
    ...
    // twin nebula glows (cyan + purple) instead of a single gold sun
    const now = performance.now();
